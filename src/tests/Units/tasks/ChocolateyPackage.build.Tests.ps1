...
Describe 'ChocolateyPackage' {
    It 'Should have exported the alias correct' {
        $taskAlias = Get-Alias -Name 'ChocolateyPackage.build.PSnake.ib.tasks'

        $taskAlias.Name | Should -Be 'ChocolateyPackage.build.PSnake.ib.tasks'
        $taskAlias.ReferencedCommand | Should -Be 'ChocolateyPackage.build.ps1'
        $taskAlias.Definition | Should -Match 'PSnake[\/|\\]\d+\.\d+\.\d+[\/|\\]tasks[\/|\\]ChocolateyPackage\.build\.ps1'
    }
}
