...
Describe 'Invoke-Pester' {
    It 'Should have exported the alias correct' {
        $taskAlias = Get-Alias -Name 'Invoke-Pester.pester.build.PSnake.ib.tasks'

        $taskAlias.Name | Should -Be 'Invoke-Pester.pester.build.PSnake.ib.tasks'
        $taskAlias.ReferencedCommand | Should -Be 'Invoke-Pester.pester.build.ps1'
        $taskAlias.Definition | Should -Match 'PSnake[\/|\\]\d+\.\d+\.\d+[\/|\\]tasks[\/|\\]Invoke-Pester\.pester\.build\.ps1'
    }
}
