...
Describe 'SetPsModulePath' -Tag x {
    ...
    BeforeAll {
        $taskAlias = Get-Alias -Name 'SetPsModulePath.build.PSnake.ib.tasks'
    ...
    It 'Should have exported the alias correct' {
        ...
        $taskAlias.Name | Should -Be 'SetPsModulePath.build.PSnake.ib.tasks'
        $taskAlias.ReferencedCommand | Should -Be 'SetPsModulePath.build.ps1'
        $taskAlias.Definition | Should -Match 'PSnake[\/|\\]\d+\.\d+\.\d+[\/|\\]tasks[\/|\\]SetPsModulePath\.build\.ps1'
    }

    Context 'When setting the PSModulePath' {
        BeforeAll {
            Mock -CommandName Set-SnakePSModulePath
        }
        ...
        It 'Should run the build task without throwing' {
            ...
            Assert-MockCalled -CommandName Set-SnakePSModulePath -Exactly -Times 1 -Scope It
        }
    }
}
