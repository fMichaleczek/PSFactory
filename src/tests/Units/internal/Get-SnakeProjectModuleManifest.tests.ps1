...
Describe 'Get-SnakeProjectModuleManifest' {
    Context 'When returning the projects module manifest' {
        Context 'When no module manifest is found' {
            ...
            It 'Should not return a module manifest' {
                InModuleScope -ScriptBlock {
                    $result = Get-SnakeProjectModuleManifest -BuildRoot (Join-Path -Path $TestDrive -ChildPath 'MyModule')
        ...
        Context 'When there are more than one .psd1 file' {
            ...
            It 'Should throw the correct error message' {
                InModuleScope -ScriptBlock {
                    { Get-SnakeProjectModuleManifest -BuildRoot (Join-Path -Path $TestDrive -ChildPath 'MyModule') } |
