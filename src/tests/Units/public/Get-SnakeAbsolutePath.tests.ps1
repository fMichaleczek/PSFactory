...
Describe 'Get-SnakeAbsolutePath' {
    Context 'When both Path and RelativeTo is a relative path' {
        BeforeAll {
            Push-Location -StackName 'Get-SnakeAbsolutePath'

            Set-Location -Path $TestDrive
        }

        AfterAll {
            Pop-Location -StackName 'Get-SnakeAbsolutePath'
        }

        It 'Should return the correct expanded path' {
            $result = PSnake\Get-SnakeAbsolutePath -Path './testResult' -RelativeTo './output'
    ...
    Context 'When Path is relative and RelativeTo is absolute' {
        It 'Should return the correct expanded path' {
            $result = PSnake\Get-SnakeAbsolutePath -Path './testResult' -RelativeTo $TestDrive
    ...
    Context 'When Path is absolute and RelativeTo is relative' {
        It 'Should return the correct expanded path' {
            $result = PSnake\Get-SnakeAbsolutePath -Path "$TestDrive/testResult" -RelativeTo './output'
