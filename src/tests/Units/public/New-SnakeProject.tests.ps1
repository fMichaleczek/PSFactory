...
BeforeDiscovery {
    $testCases = @(
        <#
            ...
            We only test that the New-SnakeProject parameters & parameter set work with the template we have defined.
        #>
        @{
...
Describe New-SnakeProject {
    Context 'Invoke plaster with correct parameters for template' {
        ...
        It 'New-Sample module should call Invoke-Plaster with test case <TestCaseName>' -ForEach $testCases {
            ...
            { PSnake\New-SnakeProject @NewSampleModuleParameters  } | Should -Not -Throw
