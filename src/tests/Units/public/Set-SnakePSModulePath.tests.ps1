BeforeAll {
    Import-Module -Name (Join-Path -Path $PSScriptRoot -ChildPath '..\helpers\SetPSnakeTaskVariableTestHelpers.psm1') -Force
    ...
    {
        $env:PSnakeRequiredModulesPath = $requiredModulesPath
        $env:PSnakeBuildModuleOutput = $buildModuleOutput
    ...
    {
        $inModuleScopeParameters = @{
            requiredModulesPath = $env:PSnakeRequiredModulesPath
            buildModuleOutput   = $env:PSnakeBuildModuleOutput
        }
    }
...
Describe 'Set-SnakePSModulePath' {
    ...
    Context 'Testing return values' {
        ...
        It 'Should return exactly one object when PassThru is specified' {
            InModuleScope -ScriptBlock {
                $result = Set-SnakePSModulePath -PSModulePath $PSModulePath -BuiltModuleSubdirectory $buildModuleOutput -RequiredModulesDirectory $requiredModulesPath -PassThru
        ...
        It 'Should return nothing when PassThru is not specified' {
            InModuleScope -ScriptBlock {
                $result = Set-SnakePSModulePath -PSModulePath $PSModulePath -BuiltModuleSubdirectory $buildModuleOutput -RequiredModulesDirectory $requiredModulesPath
    ...
    Context 'Testing ShouldProcess' {
        ...
        It 'Should not alter the PSModulePath when WhatIf is specified' {
            InModuleScope -ScriptBlock {
                ...
                Set-SnakePSModulePath -PSModulePath $PSModulePath -BuiltModuleSubdirectory $buildModuleOutput -RequiredModulesDirectory $requiredModulesPath -WhatIf
