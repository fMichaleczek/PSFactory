...
Describe 'Get-FactoryAbsolutePath' {
    Context 'When both Path and RelativeTo is a relative path' {
        BeforeAll {
            Push-Location -StackName 'Get-FactoryAbsolutePath'

            Set-Location -Path $TestDrive
        }

        AfterAll {
            Pop-Location -StackName 'Get-FactoryAbsolutePath'
        }

        It 'Should return the correct expanded path' {
            $result = PSFactory\Get-FactoryAbsolutePath -Path './testResult' -RelativeTo './output'
    ...
    Context 'When Path is relative and RelativeTo is absolute' {
        It 'Should return the correct expanded path' {
            $result = PSFactory\Get-FactoryAbsolutePath -Path './testResult' -RelativeTo $TestDrive
    ...
    Context 'When Path is absolute and RelativeTo is relative' {
        It 'Should return the correct expanded path' {
            $result = PSFactory\Get-FactoryAbsolutePath -Path "$TestDrive/testResult" -RelativeTo './output'
