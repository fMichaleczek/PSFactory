...
Describe 'Get-SnakeSourcePath' {
    Context 'When no module source path is found' {
        BeforeAll {
            Mock -CommandName Get-SnakeProjectModuleManifest
        }

        It 'Should throw the correct error message' {
            { PSnake\Get-SnakeSourcePath -BuildRoot $TestDrive } | Should -Throw -ExpectedMessage 'Module Source Path not found.'
        }
    }
    ...
    Context 'When source path is ''src'' and module manifest is not found' {
        ...
        It 'Should return the correct path' {
            $result = PSnake\Get-SnakeSourcePath -BuildRoot $TestDrive
