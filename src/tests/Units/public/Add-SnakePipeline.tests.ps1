...
BeforeDiscovery {
    $testCases = @(
        <#
            ...
            We only test that the Add-SnakeBlueprint parameters & parameter set work with the templates we have defined.
        #>
        @{
            TestCaseName = 'Build'
            NewPSnakePipelineParameters = @{
                Pipeline = 'Build'
                ProjectName =  'MyBuild'
                License = 'true'
                LicenseType = 'MIT'
...
Describe Add-SnakePipeline {
    Context 'invoke plaster with correct parameters for template' {
        BeforeAll {
            Mock -CommandName Invoke-Plaster
        }

        It 'Add-SnakePipeline should call Invoke-Plaster with test case <TestCaseName>' -ForEach $testCases {
            $NewPSnakePipelineParameters.DestinationPath = $TestDrive

            { PSnake\Add-SnakePipeline @NewPSnakePipelineParameters  } | Should -Not -Throw
