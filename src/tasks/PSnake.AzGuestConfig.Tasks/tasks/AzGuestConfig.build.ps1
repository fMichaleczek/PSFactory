...
task build_guestconfiguration_packages {
    ...
    Get-ChildItem -Path $GCPackagesPath -Directory -ErrorAction SilentlyContinue | ForEach-Object -Process {
        ...
        $GCPackageOutput = Get-SnakeAbsolutePath -Path $GCPackagesOutputPath -RelativeTo $OutputDirectory
        ...
        if ($ModuleVersion)
        {
            ...
            $GCPackageOutputPath = Get-SnakeAbsolutePath -Path $GCPackagesOutputPath -RelativeTo $OutputDirectory
