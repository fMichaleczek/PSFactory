...
task Invoke_Pester_Tests_v4 {
    ...
    # Get the vales for task variables, see https://github.com/fmichaleczek/PSnake#task-variables.
    . Set-SnakeTaskVariable

    $PesterOutputFolder = Get-SnakeAbsolutePath -Path $PesterOutputFolder -RelativeTo $OutputDirectory
    ...
    $testScriptsToOutput = $PesterScript |
        ForEach-Object -Process {
            ...
            {
                Format-SnakeHashtable -Hashtable $_
            }
            else
            {
                $_
    ...
    $CodeCoverageOutputFile = Get-SnakeCodeCoverageOutputFile @getCodeCoverageOutputFile
    ...
    $codeCoverageOutputFileEncoding = Get-SnakeCodeCoverageOutputFileEncoding -BuildInfo $BuildInfo
    ...
    if ($PesterScript.Count -gt 0)
    {
        ...
        switch ($PesterScript[0])
        {
            ...
            { $_ -is [System.Collections.Hashtable] }
            {
                ...
                {
                    Write-Build -Color 'DarkGray' -Text "      ... $(Format-SnakeHashtable -Hashtable $scriptItem)"
...
Pester:
  ...
  # If a key is not set it will be using PSnake pipeline default value.
  Configuration:
    Run:
      Path:
...
"@
    ...
    $defaultPesterParameters.Configuration.TestResult.OutputPath = Get-SnakeAbsolutePath -Path "NUnitXml_$pesterOutputFileFileName" -RelativeTo $PesterOutputFolder
    ...
    {
        $PesterConfigurationCodeCoverageOutputPath = Get-SnakeAbsolutePath -Path $PesterConfigurationCodeCoverageOutputPath -RelativeTo $PesterOutputFolder
    ...
    {
        $PesterConfigurationTestResultOutputPath = Get-SnakeAbsolutePath -Path $PesterConfigurationTestResultOutputPath -RelativeTo $PesterOutputFolder
