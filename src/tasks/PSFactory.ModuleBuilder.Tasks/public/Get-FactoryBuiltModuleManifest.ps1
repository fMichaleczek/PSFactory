...
<#
    .SYNOPSIS
        Get the module manifest from a module built by PSFactory.
    ...
    .PARAMETER OutputDirectory
        ...
        By default it is set to 'output' in a PSFactory project.
    ...
    .NOTES
        See Get-FactoryBuiltModuleBase as this command only extrapolates the Manifest file from the
        Build module base, using the ModuleName parameter.
#>
function Get-FactoryBuiltModuleManifest
{
    [CmdletBinding()]
    ...
    $BuiltModuleBase = Get-FactoryBuiltModuleBase @PSBoundParameters

    Get-FactoryAbsolutePath -Path ('{0}.psd1' -f $ModuleName) -RelativeTo $BuiltModuleBase
}
