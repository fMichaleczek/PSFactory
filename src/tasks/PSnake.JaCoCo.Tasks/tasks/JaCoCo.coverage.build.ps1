...
task Merge_CodeCoverage_Files {
    ...
    if ($CodeCoverageThreshold -gt 0)
    {
        ...
        $CodeCoverageMergedOutputFile = Get-SnakeAbsolutePath -Path $CodeCoverageMergedOutputFile -RelativeTo $PesterOutputFolder
...
task Convert_Pester_Coverage {
    ...
    $coverageXml = New-SnakeJaCoCoDocument -MissedCommands $missedCommands -HitCommands $hitCommands -PackageName $SourcePath -PackageDisplayName $ModuleVersionFolder
    ...
    Out-SnakeXml -Path $newCoverageFilePath -XmlDocument $coverageXml -Encoding $CodeCoverageOutputFileEncoding
    ...
    Out-SnakeXml -Path $codeCoverageOutputBackupFile -XmlDocument $originalXml -Encoding $CodeCoverageOutputFileEncoding
    ...
    Out-SnakeXml -Path $CodeCoverageOutputFile -XmlDocument $targetXmlDocument -Encoding $CodeCoverageOutputFileEncoding
