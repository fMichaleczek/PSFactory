<#
    ...
    .NOTES
        ...
        wrapped by the PSnake project's build.ps1 (https://github.com/fmichaleczek/PSnake).
#>
param
(
    [Parameter()]
...
task Create_Changelog_Branch {
    ...
    $ChangelogPath = Get-SnakeAbsolutePath -Path $ChangelogPath -RelativeTo $ProjectPath
    ...
    PSnake\Invoke-SnakeGit -Argument @('config', 'user.name', $GitConfigUserName)
    PSnake\Invoke-SnakeGit -Argument @('config', 'user.email', $GitConfigUserEmail)
    PSnake\Invoke-SnakeGit -Argument @('config', 'pull.rebase', 'true')
    ...
    PSnake\Invoke-SnakeGit -Argument $pullArguments
    ...
    $defaultBranchHeadCommit = PSnake\Invoke-SnakeGit -Argument @('rev-parse', "origin/$MainGitBranch")

    Write-Build DarkGray ("`tGet tags at commit '{0}'." -f $defaultBranchHeadCommit)

    $tagsAtCommit = PSnake\Invoke-SnakeGit -Argument @('tag', '-l', '--points-at', $defaultBranchHeadCommit)
    ...
    PSnake\Invoke-SnakeGit -Argument @('checkout', '-B', $branchName)
    ...
    PSnake\Invoke-SnakeGit -Argument @('add', $ChangelogFilesToAdd)

    PSnake\Invoke-SnakeGit -Argument @('commit', '-m', "Updating Changelog since $tagVersion +semver:skip")
    ...
    PSnake\Invoke-SnakeGit -Argument $pushArguments
