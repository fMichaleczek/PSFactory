...
task copy_chocolatey_source_to_staging {
    ...
    $ChocolateyPackageSource = Get-SnakeAbsolutePath -Path $ChocolateyPackageSource -RelativeTo $SourcePath
...
task copy_paths_to_choco_staging {
    ...
    foreach ($stagedPackage in $stagedPackages)
    {
        ...
        $packagePath = Get-SnakeAbsolutePath -Path $packageName -RelativeTo $ChocolateyBuildOutput
        Write-Build DarkGray "`tCopying folders to '$packageName'..."
        foreach ($copyItem in $copyToPackage)
        {
            $CopyFoldersToChocoParams = @{
                Path = Get-SnakeAbsolutePath -Path $ExecutionContext.InvokeCommand.ExpandString($copyItem.source) -RelativeTo $BuildRoot
                Destination = Get-SnakeAbsolutePath -Path $ExecutionContext.InvokeCommand.ExpandString($copyItem.destination) -RelativeTo $packagePath
            }
...
task upate_choco_nuspec_data {
    ...
    $ChangeLogOutputPath = Get-SnakeAbsolutePath -Path 'CHANGELOG.md' -RelativeTo $OutputDirectory
