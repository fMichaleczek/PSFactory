<#
    ...
    .OUTPUTS
        ...
        See https://github.com/fmichaleczek/PSnake#task-variables.
...
{
    $ProjectName = Get-SnakeProjectName -BuildRoot $BuildRoot -ErrorAction Ignore
}
...
{
    $SourcePath = Get-SnakeSourcePath -BuildRoot $BuildRoot
}

"`tSource Path                = '$SourcePath'"

$OutputDirectory = Get-SnakeAbsolutePath -Path $OutputDirectory -RelativeTo $BuildRoot

"`tOutput Directory           = '$OutputDirectory'"

$ReleaseNotesPath = Get-SnakeAbsolutePath -Path $ReleaseNotesPath -RelativeTo $OutputDirectory
...
{
    $BuiltModuleSubdirectory = Get-SnakeAbsolutePath -Path $BuiltModuleSubDirectory -RelativeTo $OutputDirectory
}
elseif ($BuildInfo.ContainsKey('BuiltModuleSubdirectory'))
{
    $BuiltModuleSubdirectory = Get-SnakeAbsolutePath -Path $BuildInfo['BuiltModuleSubdirectory'] -RelativeTo $OutputDirectory
    $BuildModuleOutput = $BuiltModuleSubdirectory
}
else {
    $BuiltModuleSubdirectory = Get-SnakeAbsolutePath -Path '' -RelativeTo $OutputDirectory
}
...
{
    $ChocolateyBuildOutput = Get-SnakeAbsolutePath -Path $ChocolateyBuildOutput -RelativeTo $OutputDirectory
...
else
{
    ...
    $ModuleManifestPath = Get-SnakeAbsolutePath -Path "$ProjectName.psd1" -RelativeTo $SourcePath
...
else
{
    ...
    $BuiltModuleManifest = Get-SnakeBuiltModuleManifest @GetBuiltModuleManifestParams
    ...
    $BuiltModuleBase = Get-SnakeBuiltModuleBase @GetBuiltModuleManifestParams
    ...
    $ModuleVersion = Get-SnakeBuiltModuleVersion @GetBuiltModuleManifestParams

    "`tModule Version             = '$ModuleVersion'"

    $moduleVersionObject = Split-SnakeModuleVersion -ModuleVersion $ModuleVersion
    ...
    if ($BuiltModuleManifest)
    {
        $BuiltModuleRootScriptPath = Get-SnakeModuleRootPath -ModuleManifestPath $BuiltModuleManifest
