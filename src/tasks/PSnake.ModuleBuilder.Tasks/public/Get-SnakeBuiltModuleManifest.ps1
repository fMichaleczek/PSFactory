...
<#
    .SYNOPSIS
        Get the module manifest from a module built by PSnake.
    ...
    .NOTES
        See Get-SnakeBuiltModuleBase as this command only extrapolates the Manifest file from the
        Build module base, using the ModuleName parameter.
#>
function Get-SnakeBuiltModuleManifest
{
    [CmdletBinding()]
    ...
    $BuiltModuleBase = Get-SnakeBuiltModuleBase @PSBoundParameters

    Get-SnakeAbsolutePath -Path ('{0}.psd1' -f $ModuleName) -RelativeTo $BuiltModuleBase
}
