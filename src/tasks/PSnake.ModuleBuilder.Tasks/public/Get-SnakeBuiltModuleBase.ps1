...
<#
    .SYNOPSIS
        Get the ModuleBase of a module built with PSnake (directory where the module
        manifest is).
    ...
    .PARAMETER OutputDirectory
        ...
        By default it is set to 'output' in a PSnake project.
    ...
    .PARAMETER ModuleName
        Name of the Module to retrieve the version from its manifest (See Get-SnakeProjectName).
...
#>
function Get-SnakeBuiltModuleBase
{
    [CmdletBinding()]
    ...
    $BuiltModuleOutputPath = Get-SnakeAbsolutePath -Path $BuiltModuleSubdirectory -RelativeTo $OutputDirectory
    $BuiltModulePath = Get-SnakeAbsolutePath -Path $ModuleName -RelativeTo $BuiltModuleOutputPath
    ...
    if ($VersionedOutputDirectory -or ($PSBoundParameters.ContainsKey('ModuleVersion') -and $ModuleVersion -ne '*'))
    {
        ...
        {
            $ModuleVersion = (Split-SnakeModuleVersion -ModuleVersion $ModuleVersion).Version
        }

        $BuiltModuleBase = Get-SnakeAbsolutePath -Path $ModuleVersion -RelativeTo $BuiltModulePath
    }
    else
    {
