...
<#
    .SYNOPSIS
        Get the module version from the module built by PSnake.

    .DESCRIPTION
        Will read the ModuleVersion and PrivateData.PSData.Prerelease tag of the Module Manifest
        that has been built by PSnake, by looking into the OutputDirectory where the Project's
    ...
    .EXAMPLE
        Get-SnakeBuiltModuleVersion -OutputDirectory 'output' -ProjectName PSnake

#>
function Get-SnakeBuiltModuleVersion
{
    [CmdletBinding()]
    ...
    $BuiltModuleManifestPath = Get-SnakeBuiltModuleManifest @PSBoundParameters
    ...
    $moduleVersionParts = Split-SnakeModuleVersion -ModuleVersion $ModuleVersion
