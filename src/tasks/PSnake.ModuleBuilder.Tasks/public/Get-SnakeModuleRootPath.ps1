...
#>
function Get-SnakeModuleRootPath
{
    [CmdletBinding()]
    ...
    $moduleInfo = Get-SnakeModuleInfo @PSBoundParameters

    if ($moduleInfo.Keys -contains 'RootModule')
    {
        Get-SnakeAbsolutePath -Path $moduleInfo.RootModule -RelativeTo (Split-Path -Parent -Path $ModuleManifestPath)
    }
    else
    {
        return $null
