...
task Build_NestedModules_ModuleBuilder {
        ...
        foreach ($paramName in $cmdParamKeys)
        {
            if ($paramName -eq 'SourcePath' -or $paramName -eq 'Path')
            {
                ...
                else
                {
                    ...
                    $nestedModuleSourceManifest = Get-SnakeAbsolutePath -Path $nestedModuleSourceManifest -RelativeTo $buildRoot
            ...
            elseif ($paramName -in @('Destination', 'OutputDirectory', 'SemVer'))
            {
                ...
                {
                    $cmdParam[$paramName] = Get-SnakeAbsolutePath -Path $cmdParam[$paramName] -RelativeTo $BuildRoot
    ...
    $ModuleInfo = Get-SnakeModuleInfo -ModuleManifestPath $builtModuleManifest
...
task Build_DscResourcesToExport_ModuleBuilder {
    ...
    $builtDscResourcesFolder = Get-SnakeAbsolutePath -Path 'DSCResources' -RelativeTo $builtModuleBase
    ...
    if ($null -ne $builtModuleRootScriptPath -and (Test-Path -Path $builtModuleRootScriptPath))
    {
        ...
        $builtClassDscResourcesNames = Get-SnakeClassBasedResourceName -Path $builtModuleRootScriptPath
    ...
    if ($builtMofDscFolder = (Get-ChildItem -Path $builtDscResourcesFolder -Directory -ErrorAction SilentlyContinue))
    {
        if ($mofPath = $builtMofDscFolder | Get-ChildItem -Filter *.schema.mof -File)
        {
            try
            {
                $builtMofDscResourcesNames = $mofPath.FullName | Get-SnakeMofSchemaName | ForEach-Object -Process {
    ...
    if ($builtMofDscFolder = (Get-ChildItem -Path $builtDscResourcesFolder -Directory -ErrorAction SilentlyContinue))
    {
        ...
        {
            $builtMofDscResourcesNames = $mofPath.FullName | Get-SnakePsm1SchemaName -ErrorAction Stop
        }
        else
        {
