...
#>
function Get-FactoryModuleRootPath
{
    [CmdletBinding()]
    ...
    $moduleInfo = Get-FactoryModuleInfo @PSBoundParameters

    if ($moduleInfo.Keys -contains 'RootModule')
    {
        Get-FactoryAbsolutePath -Path $moduleInfo.RootModule -RelativeTo (Split-Path -Parent -Path $ModuleManifestPath)
    }
    else
    {
        return $null
