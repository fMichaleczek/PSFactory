...
<#
    .SYNOPSIS
        Create a PSnake scaffolding so you can add samples & build pipeline.

    .DESCRIPTION
        Add-SnakePipeline helps you bootstrap your project pipeline, whether it's for a Chocolatey
    ...
    .PARAMETER Pipeline
        ...
        You can also create a Chocolatey pipeline, preconfigured to build Chocolatey packages, or call a PSnake pipeline.

    .EXAMPLE
        C:\src> Add-SnakePipeline -DestinationPath . -Pipeline Build -ProjectName MyBuild -License 'true' -LicenseType MIT -SourceDirectory Source  -MainGitBranch main -ModuleDescription 'some desc' -CustomRepo PSGallery -Features All

    .NOTES
        Other parameters will be displayed based on the Template used for the pipeline.
        See Add-SnakeBlueprint to add elements such as functions (private or public), tests, DSC Resources to your project.
#>
function Add-SnakePipeline
{
