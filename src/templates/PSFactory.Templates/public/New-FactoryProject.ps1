...
function New-FactoryProject
{
    ...
    $templateSubPath = 'Templates/PSFactory'
    $factoryBase = $MyInvocation.MyCommand.Module.ModuleBase

    $invokePlasterParam = @{
        TemplatePath = Join-Path -Path $factoryBase -ChildPath $templateSubPath
