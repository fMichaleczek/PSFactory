<#
    ...
    .OUTPUTS
        ...
        See https://github.com/fmichaleczek/PSFactory#task-variables.
...
{
    $ProjectName = Get-FactoryProjectName -BuildRoot $BuildRoot -ErrorAction Ignore
}
...
{
    $SourcePath = Get-FactorySourcePath -BuildRoot $BuildRoot
}
...
$ReleaseNotesPath = Get-FactoryAbsolutePath -Path $ReleaseNotesPath -RelativeTo $OutputDirectory
...
{
    $BuiltModuleSubdirectory = Get-FactoryAbsolutePath -Path $BuiltModuleSubDirectory -RelativeTo $OutputDirectory
}
elseif ($BuildInfo.ContainsKey('BuiltModuleSubdirectory'))
{
    $BuiltModuleSubdirectory = Get-FactoryAbsolutePath -Path $BuildInfo['BuiltModuleSubdirectory'] -RelativeTo $OutputDirectory
    $BuildModuleOutput = $BuiltModuleSubdirectory
}
else {
    $BuiltModuleSubdirectory = Get-FactoryAbsolutePath -Path '' -RelativeTo $OutputDirectory
}
...
else
{
    ...
    $ModuleManifestPath = Get-FactoryAbsolutePath -Path "$ProjectName.psd1" -RelativeTo $SourcePath
...
else
{
    ...
    $BuiltModuleManifest = Get-FactoryBuiltModuleManifest @GetBuiltModuleManifestParams
    ...
    $BuiltModuleBase = Get-FactoryBuiltModuleBase @GetBuiltModuleManifestParams
    ...
    if ($BuiltModuleManifest)
    {
        $BuiltModuleRootScriptPath = Get-FactoryModuleRootPath -ModuleManifestPath $BuiltModuleManifest
