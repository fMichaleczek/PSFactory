...
<#
    .SYNOPSIS
        Get the ModuleBase of a module built with PSFactory (directory where the module
        manifest is).
...
#>
function Get-FactoryBuiltModuleBase
{
    [CmdletBinding()]
    ...
    $BuiltModuleOutputPath = Get-FactoryAbsolutePath -Path $BuiltModuleSubdirectory -RelativeTo $OutputDirectory
    $BuiltModulePath = Get-FactoryAbsolutePath -Path $ModuleName -RelativeTo $BuiltModuleOutputPath
