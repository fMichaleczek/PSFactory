...
<#
    .SYNOPSIS
        Get the module version from the module built by PSFactory.

    .DESCRIPTION
        Will read the ModuleVersion and PrivateData.PSData.Prerelease tag of the Module Manifest
        that has been built by PSFactory, by looking into the OutputDirectory where the Project's
...
function Get-BuiltModuleVersion
{
    ...
    $BuiltModuleManifestPath = Get-FactoryBuiltModuleManifest @PSBoundParameters
