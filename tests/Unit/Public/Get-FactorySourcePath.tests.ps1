...
Describe 'Get-FactorySourcePath' {
    Context 'When no module source path is found' {
        BeforeAll {
            Mock -CommandName Get-FactoryProjectModuleManifest
        }

        It 'Should throw the correct error message' {
            { PSFactory\Get-FactorySourcePath -BuildRoot $TestDrive } | Should -Throw -ExpectedMessage 'Module Source Path not found.'
        }
    }
    ...
    Context 'When source path is ''src'' and module manifest is not found' {
        ...
        It 'Should return the correct path' {
            $result = PSFactory\Get-FactorySourcePath -BuildRoot $TestDrive
    ...
    Context 'When source path is ''src'' and module manifest is found' {
        BeforeAll {
            Mock -CommandName Get-FactoryProjectModuleManifest -MockWith {
                return @{
                    Directory = @{
